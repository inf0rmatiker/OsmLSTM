    ...
    with open(filename, 'r') as f:
        ...
        for tile_entry in range(4):
          ...
          if line[:8] == line_patterns[tile_entry]:
            vals = [int(x) for x in line.split(',')[4:]] # Capture counts
...
'''
def get_data_sets(window_size, max_count):
    train_features = []
    train_targets = []
    validation_features = []
    validation_targets = []
    ...
    while window_end < (len(files) - 1):
        ...
        for i in range(window_start, window_end):
            ...
            counts = [(float(x) / max_count) for x in get_counts(file)] # Normalize counts between 0 and 1
